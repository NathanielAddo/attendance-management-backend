...
const getSummaryReport = async (req, res) => {
  try {
    ...
    const attendanceData = await Attendance.aggregate([
      { $match: { userId: { $in: userIds }, date: { $gte: new Date(startDate), $lte: new Date(endDate) } } },
      { $group: {
        _id: '$userId',
...
const getBreakdownReport = async (req, res) => {
  try {
    const { userType, country, branch, category, group, subgroup, schedule, startDate, endDate, search } = req.query;
...
const downloadBreakdownReport = async (req, res) => {
  try {
    const { userType, country, branch, category, group, subgroup, schedule, startDate, endDate, search } = req.query;
